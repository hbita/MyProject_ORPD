...
function [Fbest,Lbest,BestChart]=ABC(N,max_it,testsistem,Case)
...
for i=1:FoodNumber
ObjVal(i)=Fobj(Foods(i,:),testsistem,Case);
...
%%%%%%%%% EMPLOYED BEE PHASE %%%%%%%%%%%%%%%%%%%%%%%%
    for i=1:(FoodNumber)
       ...
       %  /*if generated parameter value is out of boundaries, it is shifted onto the boundaries*/
        ...
        ObjValSol=Fobj(sol,testsistem,Case);
...
%--------------------------------------------------------------------------
%fprintf('ABC| Iter:%3.0f -->  Fbest: %9.5f\n',iter,GlobalMin); 
%--------------------------------------------------------------------------
%fprintf('Iter=%d ObjVal=%g\n',iter,GlobalMin);
GLM(iter)=GlobalMin; 
iter=iter+1;
%--------------------------------------------------------------------------
 %GLM(iter)=GlobalMin; %Prikaz konverg. ABC
 %plot(GLM,'--k','LineWidth',2);
 %title(['\fontsize{11}\bf OPF']);
 %xlabel('\fontsize{11}\bf Iteration');ylabel('\fontsize{11}\bf Fobj');
 %legend('\fontsize{11}\bf ABC',1);grid on;
 %-------------------------------------------------------------------------
end % End of ABC
Fbest=GlobalMin;
Lbest=GlobalParams;
