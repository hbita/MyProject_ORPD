function [Fbest,Lbest,BestChart]=FGO(numAgents,maxIter,testSystem,Case)
    ...
    %%---------------Initialization----------------------%%
    S=initialization(numAgents,dim,ub,lb); % Initialize the S of crested porcupines
    t=0;        % Function evaluation counter
    fit = zeros(numAgents, 1);
    %%---------------------Evaluation-----------------------%%
    for i=1:numAgents
        % fit(i)=feval(fobj, S(i,:)');
        fit(i) = obj_fun(S(i,:)',testSystem,Case);
    end
    ...
    while t<maxIter 
        ...
        if t <= M/2                         % Compute the the nutrient allocation according to (12)
            nutrients = rand(numAgents);      % Allocate more randomly to encourage fluctuation exploitation
        else
        ...
        if rand<rand    % Hyphal tip growth behavior 
            for i=1:numAgents
                ...
                Er=M+(1-t/(maxIter)).*(1-M);                      % Compute Er according to (24)
                if p<Er
                    F=(fit(i)/(sum(fit))).*rand*(1-t/(maxIter))^(1-t/(maxIter)); % Calculate F according to (5) and (6)
                ...
                nF=obj_fun(S(i,:)',testSystem,Case);
...
function Positions=initialization(numAgents,dim,ub,lb)
    ...
    if Boundary_no==1
         Positions=rand(numAgents,dim).*(ub-lb)+lb;
    end
    ...
    if Boundary_no>1
        for i=1:dim
            ub_i=ub(i);
            lb_i=lb(i);
            Positions(:,i)=rand(numAgents,1).*(ub_i-lb_i)+lb_i;      
        end
    end
end